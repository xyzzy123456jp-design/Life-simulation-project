...
> - [Codex修正→反映] Codexが`response.done`の`response.status`を検査し、`completed`以外では対象Responseの管理状態だけを破棄して`response.create`を送らない処理を追加した。
>   理由: cancelled / failed / incomplete等のResponseを根拠に会話を継続すると、失敗・中断済みResponseから不要な次Responseを生成するため。
> - [Codex修正→反映] CodexがDirect Morph方式の18個のExpression Morphを必須セットとして検査し、不足一覧をログ出力してBackendを利用不可にする処理を追加した。
>   理由: 一部Morphしか存在しない顔で、実際には何も動かないemotionを`status:"applied"`として返す誤判定を防ぐため。
> - [Codex修正→反映] Codexが正常な`response.create`送信後および異常status受信時に、Expected/Completed Call KeyとResponse固有状態をResponse単位で削除する処理を追加した。
>   理由: 長時間会話で`PendingFunctionResponses`と`GloballyCompletedFunctionCallKeys`が増え続けることを防ぎつつ、arguments.doneとresponse.doneの到着順序の両方を維持するため。
> - [Codex修正→反映] CodexがBlender Previewのhappy / surprised / sad / confused / embarrassedのMorph構成と係数をruntime C++へ完全に一致させた。
>   理由: Blenderで確認した表情とゲーム内で表示される表情の差をなくし、調整結果を再現可能にするため。
> - [Codex修正→反映] CodexがDirect Morph補間の収束状態を保持し、最終値を一度Pushした後は次のTarget受信までExpression Morph更新を停止する処理を追加した。
>   理由: 389,619頂点のJennifer顔へ収束後も毎フレーム同じMorph値を送る不要な負荷を避け、所有系統が別の`jawOpen` LipSyncは従来どおり更新し続けるため。
